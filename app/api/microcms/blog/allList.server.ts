...
export const allList = async (context: LoaderFunctionArgs["context"]) => {
  const client = createClient(context);
  const res = await client.get<MicroCMSListResponse<Blog>>(ENDPOINT.blog, {
  ...
  const contents = await [...Array(count || 2)].reduce<Promise<Blog[]>>(
    async (acc, _, i) => {
      const num = i + 1;
      const v = await client.get<MicroCMSListResponse<Blog>>(ENDPOINT.blog, {
        orders: "-publishedAt,-revisedAt",
